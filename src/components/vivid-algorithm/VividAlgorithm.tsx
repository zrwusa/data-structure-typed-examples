...
export const VividAlgorithm = function (props: VividAlgorithmProps) {
    ...
    const _handleVertexClick = (vertex: AbstractVertex) => {
        console.log(vertex);
    }

    const _handleNodeClick = (node: TreeNode) => {
        console.log(node);
    }

    const textFillColor = '#ee8800';
    const textFillActiveColor = '#ffeeff';
    const circleFillColor = '#eeeeee';
    const circleFillActiveColor = '#3489ab';
    const rectStrokeColor = '#bbbbbb';
    const arrowColor = '#6abfc1';
    const lineStrokeColor = '#bbbbbb';
    const circleStrokeColor = '#bbbbbb'

    const matrixPanelWidth = 660;
    ...
    interface LineWithArrowProps extends React.SVGProps<SVGPathElement> {
        fromV: Coordinate,
        toV: Coordinate,
        weight?: number,
        delta?: number
    ...
    const VividTreeRecursive: React.FC<{
        ...
        return (
            <g key={node.id}>
                ...
                <circle style={{cursor: 'pointer'}} onClick={() => _handleNodeClick(node)} stroke={circleStrokeColor}
